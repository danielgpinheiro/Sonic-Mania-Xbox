...
#ifdef __XBOX__
        // The heading is centred on-screen, so the fixed 188/124 offsets (authored for the
        // wider ~16:9 424 layout) push the top-anchored prompts off the edges at the Xbox
        // true-4:3 viewport (pixWidth=320). Anchor them a fixed distance from the screen
        // edges instead (matches the Dreamcast 320-wide port).
